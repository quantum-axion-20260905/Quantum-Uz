            ...
            <section className="w-full px-4 md:px-12 lg:px-20 mx-auto pb-20">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map((course, idx) => {
                        let previewVideoId = "dQw4w9WgXcQ";
                        if (course.modules && course.modules.length > 0) {
                            const url = new URL(course.modules[0].videoUrl);
                            const paths = url.pathname.split('/');
                            previewVideoId = paths[paths.length - 1] || "dQw4w9WgXcQ";
                        }
                            ...
                            <Card key={course.id} className="overflow-hidden flex flex-col group animate-in zoom-in-95 duration-700" style={{ animationDelay: `${idx * 150}ms` }}>
                                ...
                                <CardContent className="flex-1 mt-auto pb-4">
                                    <div className="flex flex-wrap gap-2">
                                        {course.tags.map(tag => (
                                            <span key={tag} className="text-[10px] text-muted-foreground bg-background border border-border px-2 py-1 rounded">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <CardFooter className="border-t border-border pt-4 pb-4 bg-muted/10">
                                    <div className="flex items-center gap-2 text-muted-foreground w-full justify-between">
                                        ...
                                        <Link href={`/academy/${course.id}`} className="text-xs text-foreground bg-background hover:bg-muted transition-colors px-3 py-1.5 rounded-lg border border-border font-medium shadow-sm">
                                            Boshlash
                                        </Link>
                                    </div>
                                </CardFooter>
