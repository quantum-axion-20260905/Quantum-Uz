                ...
                <div className="mb-10 group cursor-pointer relative lg:flex items-stretch gap-0 rounded-2xl overflow-hidden glass-panel border border-border glow-effect animate-in fade-in slide-in-from-bottom-8 duration-700 shadow-lg w-full">
                    ...
                    <div className="lg:w-3/5 p-6 lg:p-10 space-y-4 flex flex-col justify-center">
                        <div className="flex items-center gap-2 text-xs text-foreground font-medium font-inter">
                            ...
                            <span className="text-muted-foreground border-b border-muted-foreground/30 border-dashed">{scientificPapers[0]?.category}</span>
                        </div>
                        ...
                        <h2 className="text-2xl lg:text-3xl font-playfair font-bold text-foreground group-hover:text-foreground/80 transition-colors leading-tight line-clamp-3">
                            {scientificPapers[0]?.title}
                        </h2>

                        <p className="font-inter text-muted-foreground text-sm line-clamp-4 leading-relaxed bg-muted/20 p-3 rounded-lg border border-border/50 border-l-[3px] border-l-foreground shadow-sm">
                            {scientificPapers[0]?.abstract}
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-5 border-t border-border mt-auto gap-3">
                            <div className="flex items-center gap-3 text-xs text-muted-foreground font-inter">
                                <div className="flex items-center gap-1.5 bg-background px-2 py-1 rounded-md border border-border shadow-sm">
                                    ...
                                    <span className="font-medium">{scientificPapers[0]?.publishedDate}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Clock className="w-3.5 h-3.5" />
                                    <span>{scientificPapers[0]?.readTime} o'qish</span>
                                </div>
                            </div>
                            <Link href={`/journal/${scientificPapers[0]?.id}`} className="flex items-center justify-center gap-1 text-background text-xs font-semibold hover:bg-foreground/90 transition-colors bg-foreground px-4 py-2 rounded-lg shadow-sm w-full sm:w-auto shrink-0">
            ...
            <section className="w-full px-4 md:px-12 lg:px-20 mx-auto pb-20 border-t border-border pt-12 bg-muted/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {scientificPapers.map((paper, idx) => (
                        <Link href={`/journal/${paper.id}`} key={paper.id} className="group flex flex-col p-6 rounded-2xl border border-border bg-background shadow-sm hover:shadow-md hover:border-foreground/30 transition-all cursor-pointer h-full animate-in zoom-in-95 duration-700" style={{ animationDelay: `${idx * 150}ms` }}>
                            ...
                            <p className="font-inter text-[11px] md:text-xs text-muted-foreground mb-6 line-clamp-3 leading-relaxed">
                                {paper.abstract}
                            </p>

                            <div className="mt-auto bg-muted/30 p-3 rounded-lg border border-border/50 flex flex-col gap-2">
                                <div className="flex justify-between items-center w-full">
                                    <span className="text-[10px] md:text-xs font-semibold text-foreground font-inter">{paper.authors.join(", ")}</span>
                                    <span className="text-[9px] font-mono px-1.5 py-0.5 bg-background rounded border border-border text-muted-foreground border-dashed">DOI: {paper.id.split('-')[1]}</span>
                                </div>
                                <div className="flex items-center justify-between text-[10px] md:text-xs text-muted-foreground font-inter w-full">
                                    <span className="font-medium">{paper.publishedDate}</span>
                    ...
                    {/* Loop extra skeleton papers to fill grid */}
                    {[1, 2, 3, 4].map((mock, i) => (
                        <div key={`mock-${i}`} className="flex flex-col p-6 rounded-2xl border border-border border-dashed bg-muted/10 opacity-60">
                            <div className="w-full h-6 bg-muted rounded-md mb-4 animate-pulse" />
                            <div className="w-2/3 h-4 bg-muted rounded-md mb-2 animate-pulse" />
                            <div className="w-3/4 h-4 bg-muted rounded-md mb-6 animate-pulse" />
                            <div className="w-full h-16 bg-muted rounded-lg mt-auto animate-pulse" />
                        </div>
                    ))}
                </div>
            </section>
